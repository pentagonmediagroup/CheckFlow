    ...
    <div className="page-pad">
      <div style={{ display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:22,flexWrap:'wrap',gap:10 }}>
        ...
        <div style={{ display:'flex',gap:8,flexWrap:'wrap' }}>
          <button onClick={handleToggleDisabled} style={{ display:'flex',alignItems:'center',gap:5,padding:'8px 12px',background:showDisabled?'rgba(239,68,68,.08)':'rgba(75,85,99,.15)',color:showDisabled?'#F87171':'#6B7280',border:`1px solid ${showDisabled?'rgba(239,68,68,.2)':'#2D1F4E'}`,borderRadius:8,fontSize:12,cursor:'pointer' }}>
      ...
      <div className="card" style={{ overflow:'hidden' }}>
        ...
        <table style={{ width:'100%',borderCollapse:'collapse',minWidth:580 }}>
          ...
          <tbody>
            ...
            {visible.length===0&&allVisible.length===0&&<tr><td colSpan={5} style={{ padding:'48px',textAlign:'center',color:'#4B5563' }}>No clients yet.</td></tr>}
      {visible.length===0&&allVisible.length>0&&<tr><td colSpan={5} style={{ padding:'48px',textAlign:'center',color:'#4B5563' }}>No clients on this page.</td></tr>}
          </tbody>
        </table>
        </div>
      </div>
      ...
      <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:14,flexWrap:'wrap',gap:10 }}>
        ...
        <div style={{ display:'flex',gap:4 }}>
          <button onClick={()=>setPage(1)} disabled={safePage===1}
            style={{ padding:'6px 10px',background:'rgba(139,92,246,.1)',color:safePage===1?'#374151':'#A78BFA',border:'1px solid rgba(139,92,246,.2)',borderRadius:8,fontSize:12,cursor:safePage===1?'not-allowed':'pointer',opacity:safePage===1?.5:1 }}>
            «
          </button>
          <button onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={safePage===1}
            style={{ padding:'6px 12px',background:'rgba(139,92,246,.1)',color:safePage===1?'#374151':'#A78BFA',border:'1px solid rgba(139,92,246,.2)',borderRadius:8,fontSize:12,cursor:safePage===1?'not-allowed':'pointer',opacity:safePage===1?.5:1 }}>
            ← Prev
          </button>
          {Array.from({length:totalPages}).map((_,i)=>{
            const pg = i+1
            const near = Math.abs(pg-safePage)<=1 || pg===1 || pg===totalPages
            if (!near && (pg===2||pg===totalPages-1)) return <span key={pg} style={{ padding:'6px 4px',color:'#374151',fontSize:12 }}>…</span>
            if (!near) return null
            return (
              <button key={pg} onClick={()=>setPage(pg)}
                style={{ padding:'6px 12px',background:safePage===pg?'linear-gradient(135deg,#6B21A8,#4C1D95)':'rgba(139,92,246,.1)',color:safePage===pg?'#EAB308':'#A78BFA',border:`1px solid ${safePage===pg?'#6B21A8':'rgba(139,92,246,.2)'}`,borderRadius:8,fontSize:12,cursor:'pointer',fontWeight:safePage===pg?700:400 }}>
                {pg}
              </button>
            )
          })}
          <button onClick={()=>setPage(p=>Math.min(totalPages,p+1))} disabled={safePage===totalPages}
            style={{ padding:'6px 12px',background:'rgba(139,92,246,.1)',color:safePage===totalPages?'#374151':'#A78BFA',border:'1px solid rgba(139,92,246,.2)',borderRadius:8,fontSize:12,cursor:safePage===totalPages?'not-allowed':'pointer',opacity:safePage===totalPages?.5:1 }}>
            Next →
          </button>
          <button onClick={()=>setPage(totalPages)} disabled={safePage===totalPages}
            style={{ padding:'6px 10px',background:'rgba(139,92,246,.1)',color:safePage===totalPages?'#374151':'#A78BFA',border:'1px solid rgba(139,92,246,.2)',borderRadius:8,fontSize:12,cursor:safePage===totalPages?'not-allowed':'pointer',opacity:safePage===totalPages?.5:1 }}>
            »
          </button>
        </div>
      </div>
