...
export default function ClientsPage() {
  ...
  const save = async () => {
    ...
    await logAudit({ actor_username:user?.username||'system', actor_role:user?.role, action:editId?'UPDATE':'CREATE', category:'client', target_type:'client', target_name:form.name.trim(), detail:`${editId?'Updated':'Created'} client ${form.name.trim()}` })
  ...
  const del = async (id:string) => {
    ...
    const delClient = clients.find(x=>x.id===id)
    await supabase.from('clients').delete().eq('id',id)
    await logAudit({ actor_username:user?.username||'system', actor_role:user?.role, action:'DELETE', category:'client', target_type:'client', target_name:delClient?.name||id, detail:`Deleted client ${delClient?.name||id}` })
