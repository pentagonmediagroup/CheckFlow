...
import { Check, Shield, Settings2, RefreshCw, Search, Trash2, Clock, History, Download } from 'lucide-react'
...
const ACTION_COLORS: Record<string,string> = { LOGIN:'#34D399',LOGIN_FAILED:'#F87171',LOGOUT:'#9CA3AF',CREATE:'#60A5FA',UPDATE:'#F59E0B',DELETE:'#F87171',APPROVE:'#34D399' }
const CAT_COLORS: Record<string,string> = { auth:'#A78BFA',session:'#60A5FA',client:'#F59E0B',staff:'#34D399',task:'#22D3EE',pipeline:'#C084FC',calendar:'#FB923C',settings:'#9CA3AF',cashflow:'#F87171',other:'#6B7280' }
...
export default function SettingsPage() {
  ...
  const exportCSV = () => {
    ...
    const a = document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='history-log.csv'; a.click()
  }

  const exportXLSX = () => {
    // Simple tab-separated that Excel opens
    const rows = filteredHistory
    ...
    const a = document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='history-log.xls'; a.click()
  }

  const filteredLogs     = logs.filter(l => { const s=!search||[l.actor_username,l.action,l.target_name,l.detail,l.category].some(v=>String(v||'').toLowerCase().includes(search.toLowerCase())); const cat=filterCat==='all'||l.category===filterCat; const act=filterAction==='all'||l.action===filterAction; return s&&cat&&act })
  const filteredHistory  = history.filter(h => !histSearch||[h.client_name,h.service,h.payment_status,h.task_pipeline].some(v=>String(v||'').toLowerCase().includes(histSearch.toLowerCase())))
            ...
            <div className="card" style={{ overflow:'hidden' }}>
              <div style={{ overflowX:'auto' }}>
                <table style={{ width:'100%',borderCollapse:'collapse',minWidth:800 }}>
                  ...
                  <tbody>
                    {filteredHistory.length===0&&!loadingHistory&&<tr><td colSpan={9} style={{ padding:40,textAlign:'center',color:'#4B5563' }}>No history yet. Records appear here when tasks complete or sessions are removed.</td></tr>}
        ...
        <div>
          <div style={{ display:'flex',alignItems:'center',gap:10,marginBottom:14,flexWrap:'wrap' }}>
            ...
            <select style={{ ...inp,width:'auto',minWidth:130 }} value={filterCat} onChange={e=>setFilterCat(e.target.value)}>
              ...
              {['auth','session','client','staff','task','pipeline','calendar','settings','cashflow'].map(c=><option key={c} value={c}>{c.charAt(0).toUpperCase()+c.slice(1)}</option>)}
            </select>
          ...
          <div className="g4" style={{ marginBottom:14 }}>
            {[{ l:'Total Events',value:logs.length },{ l:'Logins',value:logs.filter(l=>l.action==='LOGIN').length },{ l:'Failed',value:logs.filter(l=>l.action==='LOGIN_FAILED').length },{ l:'Changes',value:logs.filter(l=>['CREATE','UPDATE','DELETE','APPROVE'].includes(l.action)).length }].map(({ l,value })=>(
          ...
          <div className="card" style={{ overflow:'hidden' }}>
            {loadingLogs?<div style={{ padding:40,textAlign:'center',color:'#4B5563' }}>Loading…</div>:filteredLogs.length===0?<div style={{ padding:40,textAlign:'center',color:'#4B5563' }}>No entries match.</div>:(
              <div style={{ overflowX:'auto' }}>
                <table style={{ width:'100%',borderCollapse:'collapse',minWidth:700 }}>
                  <thead><tr style={{ borderBottom:'1px solid #2D1F4E' }}>
                    {['Timestamp','User','Action','Category','Target','Detail'].map(h=><th key={h} style={{ padding:'8px 14px',textAlign:'left',fontSize:10,color:'#4B5563',fontWeight:500,letterSpacing:'.08em',textTransform:'uppercase',whiteSpace:'nowrap' }}>{h}</th>)}
                  </tr></thead>
                  <tbody>
                    {filteredLogs.map(log=>{
                      const ac=ACTION_COLORS[log.action]||'#9CA3AF'; const cc=CAT_COLORS[log.category]||'#6B7280'
                      ...
                      return (
                        <tr key={log.id} style={{ borderBottom:'1px solid #1A1F38' }}>
                          ...
                          <td style={{ padding:'10px 14px' }}>{log.target_name&&<div style={{ fontSize:12,color:'#D1D5DB' }}>{log.target_name}</div>}{log.target_type&&<div style={{ fontSize:10,color:'#4B5563',marginTop:1 }}>{log.target_type}</div>}</td>
                          <td style={{ padding:'10px 14px',fontSize:12,color:'#9CA3AF',maxWidth:200 }}><div style={{ overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }} title={log.detail||''}>{log.detail||'—'}</div></td>
                        </tr>
                      )
                    })}
                  </tbody>
          ...
          <div style={{ marginTop:8,fontSize:12,color:'#4B5563' }}>Showing {filteredLogs.length} of {logs.length} entries</div>
        </div>
      )}
