...
// ─── Studio B seed (from official DOCX) ──────────────────────
// Hardware: Zoom LiveTrak L-20 | Sony ZV-E10 (x3) | YoloBox Ultra | RODECaster Pro II | Rode PodMic (x4)
const STUDIO_B: SOP = {
  id: 'sop-studio-b',
  name: 'Studio B',
  description: 'Hardware: Zoom LiveTrak L-20 | Sony ZV-E10 (x3) | YoloBox Ultra | RODECaster Pro II | Rode PodMic (x4)',
  category: 'Studio Operations',
  status: 'active',
  allowSkipWithPermission: true,
  createdBy: 'system',
  createdAt: new Date().toISOString(),
  steps: [
    // SECTION 1: POWER UP SEQUENCE
    { id:'sb-01', order:1,  title:'Main Power Switch',          description:"Flip the 'I' (Main Night Switch) by the door on the Engineering Table.", isRequired:true,  estimatedMinutes:1 },
    { id:'sb-02', order:2,  title:'RODECaster Pro II',          description:'Located on the left side of the table. Power button is on the back right side (Red Button).', isRequired:true, estimatedMinutes:1 },
    { id:'sb-03', order:3,  title:'Host Time Clock',            description:'Turn on and refresh using the small side button in front of the YoloBox.', isRequired:true, estimatedMinutes:1 },
    { id:'sb-04', order:4,  title:'YoloBox Ultra',              description:'Located behind Host Timer. Hold bottom-right power button until logo appears.', isRequired:true, estimatedMinutes:2 },
    { id:'sb-05', order:5,  title:'Zoom LiveTrak L-20',         description:'Flip switch on back right-hand side to RIGHT position.', isRequired:true, estimatedMinutes:1 },
    { id:'sb-06', order:6,  title:'TV Monitor',                 description:'Turn on TV → Settings → HDMI 1.', isRequired:true, estimatedMinutes:1 },
    { id:'sb-07', order:7,  title:'Aputure Light',              description:'Set to 30% or desired intensity.', isRequired:false, estimatedMinutes:1 },
    // SECTION 2: CAMERA & BATTERY CHECK
    { id:'sb-08', order:8,  title:'Batteries',                  description:'Confirm spare batteries are fully charged.', isRequired:true, estimatedMinutes:2 },
    { id:'sb-09', order:9,  title:'Sony ZV-E10 Cameras',        description:'Verify battery level no lower than 15%.', isRequired:true, estimatedMinutes:2 },
    { id:'sb-10', order:10, title:'Camera Settings — ISO',      description:'Set ISO to 1250 on all cameras.', isRequired:true, estimatedMinutes:3 },
    // SECTION 3: YOLOBOX RECORDING SETUP
    { id:'sb-11', order:11, title:'Recording Source',           description:'Images → Video Camera (turn RED).', isRequired:true, estimatedMinutes:2 },
    { id:'sb-12', order:12, title:'Verify Settings',            description:'Program source + 1080p resolution.', isRequired:true, estimatedMinutes:2 },
    { id:'sb-13', order:13, title:'SD Card Check',              description:'Images → Settings → SD Management. Gold chip facing you.', isRequired:true, estimatedMinutes:2 },
    { id:'sb-14', order:14, title:'Monitor Mode',               description:'Ensure camera connectivity.', isRequired:true, estimatedMinutes:1 },
    // SECTION 4: AUDIO & MIC CHECK
    { id:'sb-15', order:15, title:'Mic Check',                  description:'Host speaks into Rode PodMic.', isRequired:true, estimatedMinutes:2 },
    { id:'sb-16', order:16, title:'Audio Levels',               description:'Mostly green with minor yellow, no red.', isRequired:true, estimatedMinutes:2 },
    { id:'sb-17', order:17, title:'LiveTrak AUX Connection',    description:'Black HDMI from YoloBox connected to AUX.', isRequired:true, estimatedMinutes:1 },
    { id:'sb-18', order:18, title:'No Sound Fix (if needed)',   description:'Disconnect/reconnect AUX cord if YoloBox has no sound.', isRequired:false, estimatedMinutes:1 },
    // SECTION 5: FINAL VERIFICATION
    { id:'sb-19', order:19, title:'ISO Check — All Cameras',    description:'Verify ISO on all 3 cameras.', isRequired:true, estimatedMinutes:2 },
    { id:'sb-20', order:20, title:'Night Acceptance Setting',   description:'Confirm setting is 7/50 or preset.', isRequired:true, estimatedMinutes:1 },
    { id:'sb-21', order:21, title:'Program Confirmation',       description:'YoloBox visible on TV HDMI 1.', isRequired:true, estimatedMinutes:1 },
    // END OF DAY POWER DOWN
    { id:'sb-22', order:22, title:'Stop YoloBox Recordings',    description:'Stop all active recordings on YoloBox before powering down.', isRequired:true, estimatedMinutes:1 },
    { id:'sb-23', order:23, title:'Turn Off Cameras',           description:'Power down all Sony ZV-E10 cameras.', isRequired:true, estimatedMinutes:1 },
    { id:'sb-24', order:24, title:'Turn Off Zoom LiveTrak',     description:'Flip power switch back to OFF position.', isRequired:true, estimatedMinutes:1 },
    { id:'sb-25', order:25, title:'Turn Off RODECaster',        description:'Press Red Button on back right side.', isRequired:true, estimatedMinutes:1 },
    { id:'sb-26', order:26, title:'Turn Off YoloBox',           description:'Hold power button until unit powers down.', isRequired:true, estimatedMinutes:1 },
    { id:'sb-27', order:27, title:'Turn Off Main Night Switch', description:"Flip the Main Night Switch by the door to OFF.", isRequired:true, estimatedMinutes:1 },
  ],
}
...
function getSOPs(): SOP[] {
  try {
    ...
    const existingB = stored.find(s => s.id === STUDIO_B.id)
    // Force-update Studio B if step count has changed (e.g. after a seed update)
    if (!existingB || existingB.steps.length !== STUDIO_B.steps.length) {
      const without = stored.filter(s => s.id !== STUDIO_B.id)
      const seeded = [STUDIO_B, ...without]
...
export default function TasksPage() {
  ...
  // ── File upload handler — zero npm dependencies, pure browser APIs ──
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    if (!file) return
    setFileUploading(true); setUploadError('')
    const ext = file.name.split('.').pop()?.toLowerCase()
    try {
      if (ext === 'docx') {
        // DOCX = ZIP containing word/document.xml — parse with browser's native APIs
        const arrayBuffer = await file.arrayBuffer()
        const { default: JSZip } = await import('jszip')
        const zip = await JSZip.loadAsync(arrayBuffer)
        const xmlFile = zip.file('word/document.xml')
        if (!xmlFile) { setUploadError('Could not read DOCX structure.'); return }
        const xmlText = await xmlFile.async('string')
        // Extract plain text from XML — strip all tags, split on paragraph marks
        const plain = xmlText
          .replace(/<w:p[ >]/g, '
<w:p ')
          .replace(/<[^>]+>/g, '')
...
').map((l: string) => l.trim()).filter(Boolean)
        const steps = lines
          .filter((l: string) => l.length > 2 && !/^(section|hardware|quick|end of|troubleshoot|standard operating)/i.test(l))
          .map((line: string) => {
            const clean = line.replace(/^\d+[\.\)]\s*/, '').replace(/^[-•*]\s*/, '').trim()
            return { title: clean.slice(0, 80), description: clean.length > 80 ? clean : '', isRequired: true, estimatedMinutes: '' }
          }).filter((s: any) => s.title.length > 2)
        if (steps.length) {
          setNewSteps(steps)
          if (!newSOP.name) setNewSOP(n => ({ ...n, name: file.name.replace(/\.docx$/i,'').replace(/[-_]/g,' ') }))
        } else { setUploadError('No steps found in document. Try pasting text manually below.') }
      } else if (ext === 'pdf') {
        // PDF: show a clear message — server-side parsing required for PDFs
        setUploadError('PDF parsing requires a server. Please copy the text from your PDF and paste it into the text box below.')
      } else if (ext === 'txt') {
        ...
        const steps = lines
          .map((line: string) => {
            const clean = line.replace(/^\d+[\.\)]\s*/, '').replace(/^[-•*]\s*/, '').trim()
            return { title: clean.slice(0, 80), description: '', isRequired: true, estimatedMinutes: '' }
          }).filter((s: any) => s.title.length > 2)
        if (steps.length) {
          setNewSteps(steps)
          if (!newSOP.name) setNewSOP(n => ({ ...n, name: file.name.replace(/\.txt$/i,'').replace(/[-_]/g,' ') }))
        }
      } else {
        setUploadError('Unsupported file type. Upload a .docx or .txt file.')
      }
    } catch (err: any) {
      setUploadError('Error reading file: ' + (err?.message || 'Unknown error'))
    } finally {
        ...
        <div style={{ flex:1 }}>
          ...
          {sopView === 'create' && isOwner && (
            <div style={{ maxWidth:720 }}>
              ...
              <div className="card" style={{ padding:18,marginBottom:16 }}>
                <div style={{ fontSize:12,fontWeight:700,color:'#EAB308',marginBottom:4,textTransform:'uppercase',letterSpacing:'.06em' }}>Import Steps from File or Text</div>
                <p style={{ fontSize:12,color:'#4B5563',marginBottom:14 }}>Upload a <strong style={{color:'#A78BFA'}}>.docx</strong>, <strong style={{color:'#A78BFA'}}>.pdf</strong>, or <strong style={{color:'#A78BFA'}}>.txt</strong> — or paste plain text. Each line or step becomes a checklist item.</p>

                {/* File drop zone */}
                <label style={{ display:'block',border:'2px dashed #2D1F4E',borderRadius:10,padding:'18px 14px',textAlign:'center',cursor:'pointer',marginBottom:12,background:'rgba(139,92,246,.04)',transition:'border .15s' }}
                  onDragOver={e=>{e.preventDefault();(e.currentTarget as HTMLElement).style.borderColor='#8B5CF6'}}
                  onDragLeave={e=>{(e.currentTarget as HTMLElement).style.borderColor='#2D1F4E'}}
                  onDrop={e=>{e.preventDefault();(e.currentTarget as HTMLElement).style.borderColor='#2D1F4E';const file=e.dataTransfer.files[0];if(file){const inp=document.createElement('input');inp.type='file';Object.defineProperty(inp,'files',{value:e.dataTransfer.files});handleFileUpload({target:inp} as any)}}}>
                  <input type="file" accept=".docx,.pdf,.txt" style={{ display:'none' }} onChange={handleFileUpload}/>
                  {fileUploading
                    ? <div style={{ color:'#A78BFA',fontSize:13 }}>⏳ Parsing file…</div>
                    : <>
                        <Upload size={20} style={{ color:'#4B5563',margin:'0 auto 6px' }}/>
                        <div style={{ fontSize:13,color:'#6B7280' }}>Click to upload or drag & drop</div>
                        <div style={{ fontSize:11,color:'#374151',marginTop:2 }}>.docx · .pdf · .txt</div>
                      </>
                  }
                </label>

                {uploadError && (
                  <div style={{ marginBottom:10,padding:'8px 12px',background:'rgba(239,68,68,.08)',border:'1px solid rgba(239,68,68,.2)',borderRadius:8,fontSize:12,color:'#F87171' }}>
                    ⚠ {uploadError}
                  </div>
                )}

                {/* Paste text fallback */}
                <div style={{ fontSize:11,color:'#4B5563',marginBottom:6,textTransform:'uppercase',letterSpacing:'.06em' }}>Or paste text</div>
                <textarea style={{ ...sinp,resize:'vertical' as any,minHeight:70,marginBottom:8,fontFamily:'monospace',fontSize:12 }} placeholder={"1. Step one\n2. Step two\n3. Step three…"} value={uploadText} onChange={e=>setUploadText(e.target.value)}/>
                <button onClick={parseTextIntoSteps} disabled={!uploadText.trim()||parsing}
                  style={{ display:'flex',alignItems:'center',gap:6,padding:'8px 14px',background:'rgba(139,92,246,.12)',color:'#A78BFA',border:'1px solid rgba(139,92,246,.3)',borderRadius:8,fontSize:12,cursor:'pointer',opacity:(!uploadText.trim()||parsing)?.5:1 }}>
                  <Upload size={12}/>{parsing?'Parsing…':'Import Text as Steps'}
                </button>
              </div>
