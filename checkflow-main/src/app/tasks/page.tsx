...
export default function TasksPage() {
  ...
  // ── File upload handler (DOCX text extraction via mammoth, PDF via text layer) ──
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      if (ext === 'docx') {
        // Use mammoth (available in Next.js via npm install mammoth)
        const mammoth = await import('mammoth')
        const arrayBuffer = await file.arrayBuffer()
        const result = await mammoth.extractRawText({ arrayBuffer })
        const lines = result.value.split('\n').map((l:string) => l.trim()).filter(Boolean)
        const steps = lines
          .filter((l:string) => !/^(section|hardware|quick|end of|troubleshoot)/i.test(l))
          .map((line:string) => {
            const clean = line.replace(/^[\d\.\-\*\)]+\s*/, '').trim()
            return { title: clean.slice(0, 80), description: clean.length > 80 ? clean : '', isRequired: true, estimatedMinutes: '' }
        ...
        if (steps.length) {
          setNewSteps(steps)
          if (!newSOP.name) setNewSOP(n => ({ ...n, name: file.name.replace(/\.pdf$/i,'').replace(/[-_]/g,' ') }))
        } else { setUploadError('No readable text found in PDF. Try a text-based PDF or paste content manually.') }
      } else if (ext === 'txt') {
        const text = await file.text()
        const lines = text.split('\n').map((l:string) => l.trim()).filter(Boolean)
        const steps = lines.map((line:string) => {
          const clean = line.replace(/^[\d\.\-\*\)]+\s*/, '').trim()
          return { title: clean.slice(0, 80), description: '', isRequired: true, estimatedMinutes: '' }
        }).filter((s:any) => s.title.length > 2)
        if (steps.length) setNewSteps(steps)
      } else {
        setUploadError('Unsupported file type. Upload a .docx, .pdf, or .txt file.')
      }
    } catch (err: any) {
