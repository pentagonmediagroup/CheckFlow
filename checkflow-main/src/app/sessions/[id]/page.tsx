...
import { ArrowLeft, Clock, MapPin, User, CreditCard, Package, DollarSign, Edit2 } from 'lucide-react'
    ...
    <div className="page-pad">
      ...
      <div className="card" style={{ padding:18,marginBottom:12 }}>
        <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14 }}>
          <div style={{ display:'flex',alignItems:'center',gap:8 }}><DollarSign size={14} style={{ color:'#EAB308' }}/><span style={{ fontSize:14,fontWeight:600 }}>Payment</span></div>
        ...
        {editing ? (
          <div style={{ display:'flex',flexDirection:'column',gap:10 }}>
            ...
            <div><label className="label">Notes</label><textarea style={{ ...inp,resize:'vertical' as any }} rows={2} value={edit.notes||''} onChange={e=>setE('notes',e.target.value)}/></div>
            <button onClick={saveEdit} disabled={saving} className="btn btn-primary" style={{ width:'100%',minHeight:42 }}>{saving?'Saving…':'✓ Save Changes'}</button>
          </div>
        ) : (
          <>
