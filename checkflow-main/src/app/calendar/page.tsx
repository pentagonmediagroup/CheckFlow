...
const fmtTime = (ts:string) => {
  if (!ts) return ''
  // If stored as 'YYYY-MM-DDTHH:MM:SS' (no Z), parse as local time
  const raw = String(ts)
...
export default function CalendarPage() {
  ...
  return (
    <div style={{ padding:'28px 24px',display:'flex',flexDirection:'column',height:'calc(100vh - 60px)',overflow:'hidden',position:'relative' }}>
      ...
      {popout && (
        <div ref={popoutRef}
          style={{ position:'fixed', left:Math.min(popout.x, (typeof window!=='undefined'?window.innerWidth:800)-280), top:Math.min(popout.y, (typeof window!=='undefined'?window.innerHeight:600)-240), width:270, background:'#1A1030', border:`2px solid ${popout.item._color}66`, borderRadius:14, boxShadow:'0 8px 32px rgba(0,0,0,.6)', zIndex:200, padding:16, animation:'fadeIn .12s ease' }}>
