...
export default function CalendarPage() {
  ...
  const fetchMonth = useCallback(async (showSpinner = false) => {
    ...
    // ✅ FIX 3 — deduplicate: sessions that already have a calendar_event
    // should NOT appear as a separate raw session entry.
    const linkedSessionIds = new Set(ce.map((e:any) => e.session_id).filter(Boolean));
    const orphanSessions   = sess.filter((s:any) => !linkedSessionIds.has(s.id));

    setCalEvents(ce);
    setSessions(orphanSessions);   // only sessions with NO matching calendar_event row
  ...
  // ✅ FIX 4 — Realtime subscriptions: push instead of polling
  useEffect(() => {
    const chan = supabase
      .channel(`calendar-${year}-${month}`)
      .on('postgres_changes', { event:'*', schema:'public', table:'calendar_events' }, () => fetchMonth())
      .on('postgres_changes', { event:'*', schema:'public', table:'sessions' },        () => fetchMonth())
      .subscribe();
  ...
  async function handleReschedule() {
    ...
    try {
      ...
      await supabase.from('calendar_events').update({
        ...
        description: `Rescheduled from ${isoDate(s.start_time||'')}`,
      }).eq('session_id', s.id);
      await logAudit({ actor_username: user?.username||'system', actor_role: user?.role||'owner',
