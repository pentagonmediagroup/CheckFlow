            ...
            return (
              <div key={d} onClick={()=>openDay(dateStr)}
                ...
                {items.slice(0,3).map((ev,idx)=>(
                  <div key={idx} style={{ fontSize:10,padding:'2px 5px',borderRadius:4,background:`${ev._color}22`,color:ev._color,border:`1px solid ${ev._color}44`,marginBottom:2,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }}>
                    {ev._label||ev.title}
                  </div>
                ))}
