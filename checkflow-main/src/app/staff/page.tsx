...
const BUILTIN_ROLES = ['Sales','Editor','Executive Assistant','Camera Man','Lighting','Director','Creative Director','Marketing','Social Media','Interns','Owner','Sound Engineer','Producer','Contractor']
    ...
    <div className="page-pad">
      <div style={{ display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:22,flexWrap:'wrap',gap:10 }}>
        ...
        <div style={{ display:'flex',gap:8 }}>
          ...
          {isOwner && <button onClick={() => { setForm(blank); setEditEmp(null); setShowForm(true); setErr('') }} className="btn btn-primary"><Plus size={13}/> New Employee</button>}
        </div>
      </div>
          ...
          <div className="g2">
            ...
            <div><label className="label">Access Level</label>
              <select style={inp} value={form.app_role} onChange={e => f('app_role',e.target.value)}>
                ...
                <option value="contractor">Contractor (same as Employee)</option>
      ...
      {isLimited && <div style={{ padding:'16px',background:'rgba(245,158,11,.06)',border:'1px solid rgba(245,158,11,.15)',borderRadius:10,marginBottom:14,fontSize:13,color:'#9CA3AF' }}>You can view your own profile only. Contact an owner to manage staff records.</div>}
      <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(270px,1fr))',gap:12 }}>
        {(isLimited ? employees.filter(e => e.username === user?.username) : employees).map(e => {
