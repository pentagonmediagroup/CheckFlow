...
export default function BookPage() {
  ...
  const [form, setForm] = useState({
    ...
    reschedule_date:'', notes:'', pipeline_id:'',
  })
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const endTs   = new Date(new Date(startTs).getTime() + form.duration*60000).toISOString()
      ...
      <div style={{ display:'flex',flexDirection:'column',gap:18 }}>
        ...
        <div className="card" style={{ padding:20 }}>
          ...
          <div className="g2">
            ...
            <div><label className="label">Task Pipeline</label><select style={inp} value={form.pipeline_id} onChange={e=>set('pipeline_id',e.target.value)}><option value="">Default</option>{pipelines.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
          </div>
        </div>
        ...
        <div className="card" style={{ padding:20 }}>
          ...
          {(form.total_amount>0||form.amount_paid>0) && (
            <div style={{ marginTop:14,padding:'14px 16px',background:'#0F0A1E',border:'1px solid #2D1F4E',borderRadius:12,display:'flex',gap:24,flexWrap:'wrap' }}>
              {[['Paid','#34D399',`$${form.amount_paid.toFixed(2)}`],['Total Amount','#F87171',`$${form.total_amount.toFixed(2)}`],['Balance','#EAB308',`$${balance.toFixed(2)}`]].map(([l,c,v])=>(
                <div key={String(l)}>
