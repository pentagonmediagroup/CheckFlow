...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase.from('sessions').select('id,client_name,session_type,studio,date,start_time,payment_status,amount_owed,amount_paid').gte('date', today).order('date').limit(5),
      supabase.from('tasks').select('id,client_name,task_type,stage').order('created_at').limit(10),
      supabase.from('employees').select('id,name,app_role,available,role').order('name').limit(8),
    ]).then(([{ data:se },{ data:ta },{ data:st }]) => {
      setSessions(se||[])
      setTasks(ta||[])
      setStaff(st||[])
    })
  ...
  // KPI cards — revenue ones only show for owner
  const kpiCards = [
    ...(isOwner ? [
      { label:'Collected', value:`$${kpi.collected.toFixed(0)}`, sub:'all time', Icon:DollarSign, c:'#34D399', bg:'rgba(16,185,129,.12)', b:'rgba(16,185,129,.2)' },
      { label:'Outstanding', value:`$${kpi.owed.toFixed(0)}`, sub:kpi.unpaid>0?`${kpi.unpaid} unpaid`:'all clear', Icon:AlertCircle, c:kpi.owed>0?'#F87171':'#34D399', bg:kpi.owed>0?'rgba(239,68,68,.1)':'rgba(16,185,129,.1)', b:kpi.owed>0?'rgba(239,68,68,.2)':'rgba(16,185,129,.2)' },
    ] : []),
    { label:'Upcoming', value:String(sessions.length), sub:'sessions', Icon:CalendarDays, c:'#A78BFA', bg:'rgba(139,92,246,.12)', b:'rgba(139,92,246,.2)' },
    { label:'Pipeline', value:String(tasks.length), sub:'active tasks', Icon:SquareCheckBig, c:'#22D3EE', bg:'rgba(6,182,212,.12)', b:'rgba(6,182,212,.2)' },
  ]
  ...
  return (
    <div className="page-pad">
      ...
      {/* KPI cards */}
      <div style={{ display:'grid', gridTemplateColumns:`repeat(${kpiCards.length},1fr)`, gap:10, marginBottom:16 }}>
        {kpiCards.map(({ label,value,sub,Icon,c,bg,b }) => (
          <div key={label} className="card" style={{ padding:14 }}>
            <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:8 }}>
              <div style={{ width:30, height:30, borderRadius:8, background:bg, border:`1px solid ${b}`, display:'flex', alignItems:'center', justifyContent:'center' }}>
                <Icon size={14} style={{ color:c }}/>
              </div>
              <span style={{ fontSize:11, color:'#4B5563' }}>{sub}</span>
            </div>
            <div style={{ fontSize:24, fontWeight:700, color:'#fff', letterSpacing:'-.02em' }}>{value}</div>
      ...
      <div className="g2" style={{ marginBottom:14 }}>
        ...
        <div className="card" style={{ overflow:'hidden' }}>
          ...
          {sessions.map(s => {
            const paid = s.payment_status === 'Paid in Full'
            return (
              <Link key={s.id} href={`/sessions/${s.id}`} style={{ display:'flex', alignItems:'center', gap:10, padding:'11px 16px', borderBottom:'1px solid #1A1F38', textDecoration:'none' }}>
                <div style={{ width:34, height:34, borderRadius:8, background:'rgba(139,92,246,.2)', color:'#A78BFA', border:'1px solid rgba(139,92,246,.3)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:700, flexShrink:0 }}>
                ...
                <span style={{ fontSize:11, padding:'3px 7px', borderRadius:5, fontWeight:500, flexShrink:0, background:paid?'rgba(16,185,129,.12)':'rgba(239,68,68,.1)', color:paid?'#34D399':'#F87171', border:`1px solid ${paid?'rgba(16,185,129,.25)':'rgba(239,68,68,.25)'}` }}>{s.payment_status}</span>
            ...
            {tasks.map((t,i) => {
              ...
              return (
                <div key={t.id} style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 10px', background:'#0C0F1E', border:'1px solid #1A1F38', borderRadius:8, borderLeft:`3px solid ${color}` }}>
                  ...
                  <span style={{ fontSize:10, padding:'2px 6px', borderRadius:4, background:`${color}22`, color, flexShrink:0 }}>{t.stage}</span>
                </div>
              )
            })}
          </div>
        ...
        <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
          ...
          {staff.map(s => (
            <div key={s.id} style={{ display:'flex', alignItems:'center', gap:6, padding:'6px 12px', background:'#0C0F1E', border:'1px solid #1A1F38', borderRadius:20 }}>
              <div style={{ width:6, height:6, borderRadius:'50%', background:s.available!==false?'#34D399':'#6B7280' }}/>
      ...
      <div className="g4">
        {[
          ...
          <Link key={href} href={href} style={{ display:'flex', alignItems:'center', gap:8, padding:'0 12px', minHeight:50, background:'#1A1030', border:'1px solid #2D1F4E', borderRadius:12, textDecoration:'none' }}>
            <div style={{ width:26, height:26, borderRadius:7, background:bg, border:`1px solid ${b}`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
