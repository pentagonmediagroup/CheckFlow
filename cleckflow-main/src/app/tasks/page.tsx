                    ...
                    <div key={t.id}
                      ...
                      <select
                        ...
                        onChange={async e=>{
                          ...
                          const ids = eid ? [...new Set([...(t.assigned_staff_ids||[]),eid])] : t.assigned_staff_ids||[]
                          await supabase.from('tasks').update({ assigned_to:eid, assigned_staff_ids:ids }).eq('id',t.id)
                          setTasks(ts=>ts.map(x=>x.id===t.id?{...x,assigned_to:eid,assigned_staff_ids:ids}:x))
                        }}
                        style={{ marginTop:4,width:'100%',background:'rgba(139,92,246,.07)',border:'1px solid rgba(139,92,246,.2)',borderRadius:6,padding:'3px 6px',fontSize:10,color:'#A78BFA',outline:'none',cursor:'pointer',appearance:'none',WebkitAppearance:'none' }}>
