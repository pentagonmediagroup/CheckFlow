...
const DELIVERABLES = ['Full Mix','Stems','Video','Social Clips','Thumbnails','Audio Bounce','Photo Selects','Interview Recording','Other']
...
export default function BookPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (existing) {
        ...
        const { data: nc } = await supabase.from('clients').insert({ name: form.client_name.trim(), email: form.email||null, phone: form.phone||null, ig: form.ig||null }).select('id').single()
        clientId = nc?.id || null
      }
      ...
      // Link employees to junction table
      const empIds = [form.emp1, form.emp2, form.emp3].filter(Boolean)
      if (empIds.length) {
        await supabase.from('session_employees').insert(empIds.map(eid => ({ session_id: session!.id, employee_id: eid })))
      }

      // Auto-create ONE task (first stage of Default pipeline)
      ...
      // Add ONE calendar event — not duplicated
      ...
      setErr(e?.message || 'Error saving session')
    } finally {
      setSaving(false)
    }
  }
  ...
  if (saved) return (
    <div style={{ padding:'80px 24px', textAlign:'center' }}>
      ...
      <p style={{ color:'#6B7280', marginTop:6 }}>Saved · Task created · Added to Calendar</p>
    </div>
  )
  ...
  return (
    <div className="page-pad">
      ...
      {err && <div style={{ marginBottom:14, padding:'10px 14px', background:'rgba(239,68,68,.08)', border:'1px solid rgba(239,68,68,.2)', borderRadius:10, color:'#F87171', fontSize:13 }}>{err}</div>}
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:18 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:14 }}>
            {['Studio A','Studio B'].map(s => (
              <button key={s} type="button" onClick={() => set('studio', s)}
                style={{ padding:12, borderRadius:10, fontSize:14, fontWeight:600, cursor:'pointer', border:'1px solid', transition:'all .1s',
                  background: form.studio===s ? 'linear-gradient(135deg,#6B21A8,#4C1D95)' : '#0F0A1E',
                  color: form.studio===s ? '#EAB308' : '#9CA3AF',
                  borderColor: form.studio===s ? '#6B21A8' : '#2D1F4E' }}>{s}</button>
            ))}
          </div>
          <div className="g2">
            <div>
            ...
            <div>
              <label className="label">Employee 1</label>
              <select style={inp} value={form.emp1} onChange={e => set('emp1', e.target.value)}>
            ...
            <div>
              ...
              <select style={inp} value={form.emp2} onChange={e => set('emp2', e.target.value)}>
                <option value="">— Select —</option>
                {employees.map(e => <option key={e.id} value={e.id}>{e.name}{e.role ? ` · ${e.role}` : ''}</option>)}
              </select>
            </div>
            <div>
              <label className="label">Employee 3</label>
              <select style={inp} value={form.emp3} onChange={e => set('emp3', e.target.value)}>
                <option value="">— Select —</option>
                {employees.map(e => <option key={e.id} value={e.id}>{e.name}{e.role ? ` · ${e.role}` : ''}</option>)}
              </select>
            </div>
            <div>
              <label className="label">Salesperson (commission)</label>
        ...
        <div className="card" style={{ padding:20 }}>
          ...
          {(form.total_amount > 0 || form.amount_paid > 0) && (
            <div style={{ marginTop:16, padding:'14px 16px', background:'#0F0A1E', border:'1px solid #2D1F4E', borderRadius:12, display:'flex', gap:24, flexWrap:'wrap' }}>
              {[['Paid','#34D399',`$${form.amount_paid.toFixed(2)}`],['Total Owed','#F87171',`$${form.total_amount.toFixed(2)}`],['Balance','#EAB308',`$${balance.toFixed(2)}`],...(form.late_fee>0?[['Late Fee','#FB923C',`$${form.late_fee.toFixed(2)}`]]:[] as any[])].map(([l,c,v])=>(
                <div key={String(l)}>
        ...
        <button onClick={handleSubmit} disabled={saving || !form.client_name.trim() || !form.date || !form.start_time}
          className="btn btn-primary" style={{ width:'100%', minHeight:50, fontSize:16 }}>
